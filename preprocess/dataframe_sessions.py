...
def get_sessions_old(experiment_type: str = None) -> pd.DataFrame:
...
def process_session(session_path, session_to_type, experiment_type, frame_limits, file_patterns):
    session_date, mouse_id, day_index = session_path.parts[-3:]
    session_key = f"{session_date}/{mouse_id}/{day_index}"
...
def get_sessions(raw_path: Path, experiment_type: str = None) -> Tuple[pd.DataFrame, pd.DataFrame]:
    matches = Path(raw_path).glob('[0-9][0-9][0-9][0-9][0-9][0-9]/NVI*/D*')
    session_paths = [p for p in matches if p.is_dir()]
    session_types = get_session_paths()
