...
def get_sessions(experiment_type: str) -> pd.DataFrame:
    experiment_types = {
        'hE2_rew': _hE2_rew,
        'hE2_norew': _hE2_norew,
        'hE3_rew': _hE3_rew,
    ...
    for mouse_name, session in experiment_dict[experiment_type].items():
        for day_index, session_path in enumerate(sessions):
                        ...
                        for key, category in category_map.items():
                            ...
                            if im_subfolder.startswith(key):
                                if not os.listdir(im_subfolder_path):
                                    ...
                                    ret[f'flag_session_more_tif_{category}'][-1] = True
                                    continue
                                im_files = [subfolder for subfolder in os.listdir(im_subfolder_path) if
                                            subfolder.startswith(f"{key}_")]
                                parsed_folder_name = f"{key}/{im_files[0]}" if im_files else None
                                # Check if multiple files exist
                                if len(im_files) > 1 or not im_files:
                                    ret[f'flag_{category}'][-1] = True  # Flag session
                                    ...
                                    parsed_folder = im_subfolder_path / im_files[0]
                                    num_tiff_files = sum(
                                        1 for file in os.listdir(parsed_folder) if file.endswith(".tif"))

                                    ret[f'flag_session_more_tif_{category}'][-1] = True if num_tiff_files > tiff_limits[
                                        key] else False
                                else:
                                    ret[category][-1] = None
                                    ret[f'flag_{category}'][-1] = False  # No flag needed
