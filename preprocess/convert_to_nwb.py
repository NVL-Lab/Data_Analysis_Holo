...
def get_holo_light_mod():
    return SpatialLightModulator(
        name='No SLM',
        description='sequential stimulation',
        #model=na_light_modulator,
        resolution=0.  # float
    )

def convert_behavior_to_nwb(row: pd.DataFrame, folder_nwb_mice: str, folder_raw: str):
    # =====================================================================================================
    #                   Behavior
    # =====================================================================================================
    print('Behavior: initiating and creating the nwbfile')
    # convert data and open file
    main_folder = Path(folder_raw) / row.session_date / row.mice_name

    # No video for pre and post - not part of experiment
    avi_paths = {
        'baseline': list(Path(main_folder / 'base').rglob("*.avi"))[0],
        'pretrain': list(Path(main_folder / 'pretrain').rglob("*.avi"))[0],
        'bmi': list(Path(main_folder / 'bmi').rglob("*.avi"))[0],
    }

    save_paths = {
        'baseline': f'{folder_nwb_mice / row.mice_name}_{row.session_date}_baseline_behavior.nwb',
        'pretrain': f'{folder_nwb_mice / row.mice_name}_{row.session_date}_pretrain_behavior.nwb',
        'bmi': f'{folder_nwb_mice / row.mice_name}_{row.session_date}_bmi_behavior.nwb'
            }

    for expt in avi_paths:
        print(f'Converting {expt} behavior video')
        # Creates ImageSeries that points to the raw video paths
        #interface = ExternalVideoInterface(file_paths=Path([avi_paths[expt]).relative_to(folder_nwb_mice)], verbose=False, video_name=expt)
        interface = ExternalVideoInterface(file_paths=[avi_paths[expt]], verbose=False, video_name=expt)
        metadata = interface.get_metadata()
        # For data provenance we add the time zone information to the conversion
        session_start_time = datetime.strptime(row.session_date, "%y%m%d").replace(tzinfo=ZoneInfo("US/Pacific"))
        metadata["NWBFile"].update(session_start_time=session_start_time)
        # Add subject information (required for DANDI upload)
        metadata["Subject"] = dict(subject_id=row.mice_name, species="Mus musculus", sex="M", age="P30D")
        # Optional
        video_metadata = {
            "Behavior": {
                "ExternalVideos": {
                    expt: {  # This should match the video_name used in the interface
        ...
        interface.run_conversion(nwbfile_path=save_paths[expt], metadata=metadata, overwrite=True)
        print('Done')

def convert_bmi_to_nwb(row: pd.DataFrame, folder_nwb_mice: str, folder_raw: str):
    # =====================================================================================================
    #                   BMI
    # =====================================================================================================
    ...
    folder_bmi_im = Path(folder_raw) / row.session_path / 'im' / row.bmi_im
    nwbfile_bmi_path = f'{folder_nwb_mice / row.mice_name}_{row.session_date}_bmi.nwb'
    microscope_bmi = get_microscope()
    convert_bruker_images_to_nwb(folder_bmi_im, microscope_bmi, nwbfile_bmi_path)
    io_bmi = NWBHDF5IO(nwbfile_bmi_path, mode='a')
    nwbfile_bmi = io_bmi.read()
    frame_rate = nwbfile_bmi.acquisition['TwoPhotonSeries'].rate
    size_of_recording = nwbfile_bmi.acquisition['TwoPhotonSeries'].data.shape[0]

    nwbfile_bmi.add_device(microscope_bmi)

    print('BMI: retrieving online data')
    bmi_calibration_mat = loadmat(folder_raw / row.session_path / row.target_calibration_mat_file) # Same as pretrain
    bmi_rois_mat = loadmat(folder_raw / row.session_path / row.roi_mat_file)['roi_data'] # Same as pretrain
    ensemble_indices = bmi_calibration_mat['E_base_sel'].flatten() # Same as pretrain
    bmi_mat = loadmat(folder_raw / row.session_path / row.bmi_mat_file)['data']
    bmi_data = bmi_mat['bmiAct'].item()
    bmi_data = bmi_data[:, :np.where(~np.isnan(bmi_data).all(axis=0))[0][-1]].T
    voltage_recording = folder_bmi_im / row.bmi_im_voltage_file
    ...
    bmi_calibration = Calibration_metadata(
        ...
        feedback_flag=False,  # bool
        ensemble_indexes=ensemble_indices,
        decoder=bmi_calibration_mat['decoder'].flatten(),
        target=bmi_calibration_mat['T'].flatten(),
        ensemble_mean=bmi_calibration_mat['n_mean'].flatten(),
        ensemble_sd=bmi_calibration_mat['n_std'].flatten()
    )
    nwbfile_bmi.add_lab_meta_data(bmi_calibration)

    # Same as BMI
    conditions_target = np.full((3, max(1, bmi_calibration_mat['E2_subord_mean'].flatten().size)), np.nan, dtype=float)
    conditions_target[0, 0] = bmi_calibration_mat['T'].item()
    conditions_target[1, 0] = bmi_calibration_mat['E1_mean'].item()
    conditions_target[2, :] = bmi_calibration_mat['E2_subord_mean'].flatten()
    bmi_parameters = Parameters_BMI(
        name='BMI_parameters',
        description='Parameters used to run the BMI',
        back_to_baseline_frames=bmi_calibration_mat['back2BaseFramesThresh'].item(),
        prefix_window_frames=bmi_calibration_mat['prefix_win'].item(),
        dff_baseline_window_frames=bmi_calibration_mat['f0_win'].item(),
        smooth_window_frames=bmi_calibration_mat['dff_win'].item(),
        cursor_zscore_bool=False,
        relaxation_window_frames=0,
        timeout_window_frames=0,
        back_to_baseline_threshold=bmi_calibration_mat['b2base_thresh'].flatten(),
        conditions_rule=np.array(['cursor >= c1', 'The mean of E1 <= c2', 'At least 3 out of 4 E2 neurons >= c3'],
                                 dtype='str'),
        conditions_target=conditions_target,
        frames_per_reward_range=bmi_calibration_mat['frames_per_reward_range'].flatten().astype(int)
    )
    ...
    if 'fb' in row.experiment_type.lower():
        experiment_bmi = 'audio_feedback'
        cursor_audio = bmi_mat['fb_freq'].item().flatten()
        if bmi_calibration_mat['fb_settings']['target_low_freq'].item().item():
            freq = bmi_calibration_mat['fb_settings']['freq_min'].item().flatten()
        else:
            freq = bmi_calibration_mat['fb_settings']['freq_max'].item().flatten()
    ...
    # retrieve timeseries data from BMI - similar to pretrain
    bmi_series = CaBMISeries(
        name='BMI_online_results',
        description='Time series results of the CaBMI experiment',
        experiment_type=experiment_bmi,
        ...
        raw_activity=bmi_data,
        baseline_vector=bmi_mat['baseVector'].item(),
        self_hits=bmi_mat['selfHits'].item().flatten().astype(bool),
        self_reward=bmi_mat['selfVTA'].item().flatten().astype(bool),
        trial_start=bmi_mat['trialStart'].item().flatten().astype(bool),
        time_vector=bmi_mat['timeVector'].item().flatten()
    )
    nwbfile_bmi.add_acquisition(bmi_series)
    ...
    # retrieve the ROI data - same as pretrain
    bmi_roi = ROI_metadata(
        name='ROI_metadata',
        description='Location of the ROIs, in a binary 2D array',
        image_mask_roi=np.stack(
            bmi_rois_mat['roi_bin_cell'].item().flatten())[ensemble_indices, :, :][:, :, :, np.newaxis])
...
def convert_pretrain_to_nwb(row: pd.DataFrame, folder_nwb_mice: str, folder_raw: str):
    # =====================================================================================================
    #                   Pretrain
    # =====================================================================================================
    ...
    folder_pretrain_im = Path(folder_raw) / row.session_path / 'im' / row.pretrain_im
    nwbfile_pretrain_path = f'{folder_nwb_mice / row.mice_name}_{row.session_date}_pretrain.nwb'
    microscope_pretrain = get_microscope()
    light_source_pretrain = get_holo_light_source()
    light_mod_pretrain = get_holo_light_mod()
    convert_bruker_images_to_nwb(folder_pretrain_im, microscope_pretrain, nwbfile_pretrain_path)
    io_pretrain = NWBHDF5IO(nwbfile_pretrain_path, mode='a')
    nwbfile_pretrain = io_pretrain.read()
    frame_rate = nwbfile_pretrain.acquisition['TwoPhotonSeries'].rate
    size_of_recording = nwbfile_pretrain.acquisition['TwoPhotonSeries'].data.shape[0]

    nwbfile_pretrain.add_device(microscope_pretrain)
    ...
    if row.experiment_type[0] == 'h':
        ...
        # if there was stim, add the devices
        nwbfile_pretrain.add_device(light_source_pretrain)
        nwbfile_pretrain.add_device(light_mod_pretrain)
        ...
        for pt in np.arange(len(stim_point)):
            ...
            pretrain_stim_series = PatternedOptogeneticSeries(
                ...
                description='One cell at a time stimulation',
                site=pretrain_stim_site,
                device=microscope_pretrain,
                light_source=light_source_pretrain,
                spatial_light_modulator=light_mod_pretrain,
                stimulus_pattern=pretrain_stim_pattern,
                center_rois=np.expand_dims(np.array([loc_x[pt], loc_y[pt], spiral_size[pt]]), axis=0)
            )
            nwbfile_pretrain.add_acquisition(pretrain_stim_series)
    ...
    conditions_target = np.full((3, max(1, pretrain_calibration_mat['E2_subord_mean'].flatten().size)), np.nan, dtype=float)
    conditions_target[0, 0] = pretrain_calibration_mat['T'].item()
    conditions_target[1, 0] = pretrain_calibration_mat['E1_mean'].item()
    conditions_target[2, :] = pretrain_calibration_mat['E2_subord_mean'].flatten()
    pretrain_parameters = Parameters_BMI(
        name='BMI_parameters',
        description='Parameters used to run the BMI',
        back_to_baseline_frames=pretrain_calibration_mat['back2BaseFramesThresh'].item(),
    ...
    pretrain_results_series = CaBMISeries(
        ...
        raw_activity=pretrain_data,
        baseline_vector=pretrain_mat['baseVector'].item(),
        self_hits=pretrain_mat['selfHits'].item().flatten().astype(bool),
        stim_hits=pretrain_mat['holoHits'].item().flatten().astype(bool),
        self_reward=pretrain_mat['selfVTA'].item().flatten().astype(bool),
        stim_reward=pretrain_mat['holoVTA'].item().flatten().astype(bool),
        stim_delivery=pretrain_mat['holoDelivery'].item().flatten().astype(bool),
        trial_start=pretrain_mat['trialStart'].item().flatten().astype(bool),
        time_vector=pretrain_mat['timeVector'].item().flatten(),
        scheduled_stim=pretrain_mat['vectorHolo'].item().flatten(),
        scheduled_reward=pretrain_mat['vectorVTA'].item().flatten(),
    )
...
def convert_baseline_to_nwb(row: pd.DataFrame, folder_nwb_mice: str, folder_raw: str):
    # =====================================================================================================
    #                   Baseline
    # =====================================================================================================
    ...
    folder_baseline_im = Path(folder_raw) / row.session_path / 'im' / row.baseline_im
    nwbfile_baseline_path = f'{folder_nwb_mice / row.mice_name}_{row.session_date}_baseline.nwb'
    microscope_baseline = get_microscope()
    convert_bruker_images_to_nwb(folder_baseline_im, microscope_baseline, nwbfile_baseline_path)
    io_baseline = NWBHDF5IO(nwbfile_baseline_path, mode='a')
    nwbfile_baseline = io_baseline.read()
    frame_rate = nwbfile_baseline.acquisition['TwoPhotonSeries'].rate
    size_of_recording = nwbfile_baseline.acquisition['TwoPhotonSeries'].data.shape[0]

    nwbfile_baseline.add_device(microscope_baseline)
...
def convert_holoseq_to_nwb(row: pd.DataFrame, folder_nwb_mice: str, folder_raw: str):
    # =====================================================================================================
    #                   Holographic sequential
    # =====================================================================================================
    ...
    folder_holo_seq_im = Path(folder_raw) / row.session_path / 'im' / row.holostim_seq_im
    nwbfile_holograph_seq_path = f'{folder_nwb_mice / row.mice_name}_{row.session_date}_holostim_seq.nwb'
    microscope_holo = get_microscope()
    light_source_holo = get_holo_light_source()
    light_mod_holo = get_holo_light_mod()
    convert_bruker_images_to_nwb(folder_holo_seq_im, microscope_holo, nwbfile_holograph_seq_path)
    io_holographic_seq = NWBHDF5IO(nwbfile_holograph_seq_path, mode='a')
    nwbfile_holographic_seq = io_holographic_seq.read()
    frame_rate = nwbfile_holographic_seq.acquisition['TwoPhotonSeries'].rate
    size_of_recording = nwbfile_holographic_seq.acquisition['TwoPhotonSeries'].data.shape[0]
    ...
    # add the devices for holographic stim
    nwbfile_holographic_seq.add_device(microscope_holo)
    nwbfile_holographic_seq.add_device(light_source_holo)
    nwbfile_holographic_seq.add_device(light_mod_holo)
    ...
    for pt in np.arange(len(stim_point)):
        ...
        holo_seq_series = PatternedOptogeneticSeries(
            ...
            description='One cell at a time stimulation',
            site=holo_stim_site,
            device=microscope_holo,
            light_source=light_source_holo,
            spatial_light_modulator=light_mod_holo,
            stimulus_pattern=holo_stim_pattern,
            center_rois=np.expand_dims(np.array([loc_x[pt], loc_y[pt], spiral_size[pt]]), axis=0)
        )
        nwbfile_holographic_seq.add_acquisition(holo_seq_series)
...
def convert_experiment_to_nwb(index: int, folder_save: str, folder_raw: str, behavior_folder_raw: str, experiment_type: Optional[str]=None):
    ...
    folder_nwb = folder_save / 'nwb'

    # TODO: Add behavior!
    # If any flag represents the session can not be process normally
    row = df_sessions.loc[index]
    flag_cols = df_sessions.filter(like='Flag').columns
    if row[flag_cols].any():
        print("Flagged session: cannot convert to nwb")
        exit()
    ...
    #convert_holoseq_to_nwb(row, folder_nwb_mice, folder_raw)
    #convert_baseline_to_nwb(row, folder_nwb_mice, folder_raw)
    #convert_pretrain_to_nwb(row, folder_nwb_mice, folder_raw)
